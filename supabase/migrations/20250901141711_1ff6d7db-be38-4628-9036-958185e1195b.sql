...
-- First drop policies that reference user_role
DROP POLICY IF EXISTS "Planners can update request status and claim requests" ON public.planner_requests;
DROP POLICY IF EXISTS "Helpers and requesters can view helper requests" ON public.helper_requests;
...
-- Recreate the planner policy without helper references
CREATE POLICY "Planners can update request status and claim requests" 
ON public.planner_requests 
FOR UPDATE 
USING (EXISTS (
  SELECT 1 FROM profiles p 
  WHERE p.user_id = auth.uid() AND p.user_role = 'planner'::user_role
))
WITH CHECK (EXISTS (
  SELECT 1 FROM profiles p 
  WHERE p.user_id = auth.uid() AND p.user_role = 'planner'::user_role
