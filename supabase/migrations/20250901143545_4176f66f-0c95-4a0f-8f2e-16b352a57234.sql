-- Update planner_requests to support direct workflow
-- Add function to auto-create invoices when planner requests are approved
...
-- Create trigger for planner request approval
DROP TRIGGER IF EXISTS on_planner_request_status_change ON public.planner_requests;
CREATE TRIGGER on_planner_request_status_change
  AFTER UPDATE ON public.planner_requests
  FOR EACH ROW
  EXECUTE FUNCTION public.handle_planner_request_approval();