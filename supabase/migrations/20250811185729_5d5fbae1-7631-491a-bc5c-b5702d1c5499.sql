-- Helper Invoices Workflow Migration
...
-- 5) Policies
-- Helpers can view their invoices
CREATE POLICY IF NOT EXISTS "Helpers can view their helper invoices"
...
-- Planners can view their invoices
CREATE POLICY IF NOT EXISTS "Planners can view their helper invoices"
ON public.helper_invoices
FOR SELECT
USING (
  planner_id = (
...
-- Helpers can update their invoices (transitions enforced via trigger)
CREATE POLICY IF NOT EXISTS "Helpers can update their helper invoices"
ON public.helper_invoices
FOR UPDATE
USING (
  helper_id = (
...
-- Planners can update their invoices (transitions enforced via trigger)
CREATE POLICY IF NOT EXISTS "Planners can update their helper invoices"
ON public.helper_invoices
FOR UPDATE
USING (
  planner_id = (
...
-- No INSERT policy: created by trigger, not by end users
...
BEGIN
  ...
  -- Allowed transitions
  -- helper: draft -> sent_to_planner
  -- system: sent_to_planner -> awaiting_payment (normalize)
  -- planner: awaiting_payment -> paid_planner
  -- helper: paid_planner -> completed

  IF old_status = 'draft' AND new_status = 'sent_to_planner' THEN
    NEW.sent_at := COALESCE(NEW.sent_at, now());
    -- Normalize to awaiting_payment so planner immediately sees correct state
...
DECLARE
  ...
  amt NUMERIC;
BEGIN
  -- Only on status change to approved/accepted
  IF TG_OP = 'UPDATE' AND OLD.status IS DISTINCT FROM NEW.status AND NEW.status::text IN ('approved','accepted') THEN
    ...
    -- In-app notification (message) to helper
