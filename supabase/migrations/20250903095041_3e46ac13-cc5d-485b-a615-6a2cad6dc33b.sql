-- First, let me update the handle_planner_request_approval function to also create invoices
-- Since we're using planner_requests for approvals, not planner_applications
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' AND OLD.status != 'approved' AND NEW.status = 'approved' THEN
    ...
    -- CREATE INVOICE - This was missing from the original function!
    -- Check if invoice already exists to avoid duplicates
    SELECT EXISTS (
      SELECT 1 FROM public.planner_invoices pi WHERE pi.planner_request_id = NEW.id
    ...
    IF client_user_id IS NOT NULL AND planner_user_id IS NOT NULL THEN
      ...
      IF NOT dup_msg THEN
        ...
        VALUES (
          ...
          client_user_id,
          'Request Approved',
          'Great news! Your request "' || NEW.title || '" has been approved. An invoice has been created for you to review.'
        );
      END IF;
    END IF;
