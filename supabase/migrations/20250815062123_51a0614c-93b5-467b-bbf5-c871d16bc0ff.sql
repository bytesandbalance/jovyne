-- 1) Make helper application approval function idempotent and dedupe messages
...
-- 2) Ensure only one trigger exists on helper_applications for this function
DO $$
DECLARE r record;
BEGIN
  FOR r IN
...
-- 3) Add uniqueness to prevent multiple invoices per application (only when application id present)
...
-- 4) Dedupe messages in invoice transitions to avoid accidental duplicates
