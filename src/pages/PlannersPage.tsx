      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-4">
            {userProfile?.user_role === 'planner' ? 'Planner Hub' : 'Find Party Planners'}
          </h1>
          <p className="text-xl text-muted-foreground">
            {userProfile?.user_role === 'planner' 
              ? 'Browse planners and manage client requests'
              : 'Discover amazing party planners in your area'
            }
          </p>
        </div>

        {/* Universal tabbed view for all users */}
        <Tabs defaultValue="browse" className="space-y-6">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="browse">Browse Planners</TabsTrigger>
            <TabsTrigger value={userProfile?.user_role === 'planner' ? 'requests' : 'my-requests'}>
              {userProfile?.user_role === 'planner' ? 'Client Requests' : 'My Requests'}
            </TabsTrigger>
          </TabsList>
          ...
          {/* Client Requests Tab - for Planners */}
          {userProfile?.user_role === 'planner' && (
                              ...
                              <div className="flex items-center gap-2 mt-2">
                                <Avatar className="w-6 h-6">
                                  ...
                                  <AvatarFallback className="text-xs">
                                    {getInitials(request.planners.profiles?.full_name || request.planners.business_name)}
                                  </AvatarFallback>
                                </Avatar>
                                <span className="text-sm">
                                  Assigned to {request.planners.business_name}
                                </span>
                              </div>
                            )}
                          </div>
